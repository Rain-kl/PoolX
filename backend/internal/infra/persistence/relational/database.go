...
func OpenSQLite(ctx context.Context, path string) (*Database, error) {
	const (
		sqliteDirPerm     = 0o700
		sqliteDefaultPool = 16
	)
	if err := os.MkdirAll(filepath.Dir(path), sqliteDirPerm); err != nil {
	...
	return configureDatabase(ctx, db, "sqlite", sqliteDefaultPool, sqliteDefaultPool)
}
