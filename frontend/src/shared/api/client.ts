...
const refreshLockName = 'poolx:admin-session-refresh';
...
async function parseResponse<T>(
  ...
  // Wavelet envelope: { error_msg: string, data: T | null }
  ...
  if (!response.ok || errorMsg) {
    const code = !response.ok ? `http_${response.status}` : 'requestFailed';
    throw new ApiError(
      response.status || 400,
      code,
      localizedErrorMessage(code, errorMsg || `HTTP ${response.status}`),
    );
  }
  try {
...
async function requestRefresh(): Promise<RefreshResult> {
  try {
    const response = await fetch(
      `${runtimeConfig.apiBaseUrl}/api/v1/user/refresh`,
      {
        method: 'POST',
        credentials: 'include',
...
export async function apiEventStream<T>(
  ...
  const dispatch = (block: string) => {
    ...
    block.split('\n').forEach((line) => {
      ...
      if (normalized.startsWith('data:'))
        data.push(normalized.slice(5).trimStart());
    });
  ...
  try {
    for (;;) {
      const { done, value } = await readEventStreamChunk(
        reader,
        response.status,
      );
    ...
    timeout = window.setTimeout(() => {
      reject(
        new ApiError(
          ...
          localizedErrorMessage(
            'streamTimeout',
            'The progress stream stopped responding',
          ),
        ),
      );
...
export async function apiDownload(
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      response.status,
      'requestFailed',
      localizedErrorMessage('requestFailed', 'The request failed'),
    );
  }
  return response.blob();
}

/** Wavelet-aligned user payload (PoolX admin is always is_admin). */
...
export type LoginResponseDTO = AdminDTO & {
  tokens: AuthTokensDTO;
};

const authTokensValidator = hasShape({
  access_token: isString,
  access_token_expires_at: isString,
  refresh_token_expires_at: isString,
});

export const decodeAdminDTO = createObjectDecoder<AdminDTO>('user', {
  id: isNumber,
  username: isString,
  is_admin: (value) => typeof value === 'boolean',
});
export const decodeAuthTokensDTO = createObjectDecoder<AuthTokensDTO>(
  'auth tokens',
  {
    access_token: isString,
    access_token_expires_at: isString,
...
export const decodeLoginResponseDTO = createObjectDecoder<LoginResponseDTO>(
  'login',
  {
    id: isNumber,
    username: isString,
...
/** Logout returns empty string data (Wavelet-style). */
export const decodeLoggedOut: ApiDecoder<string | null> = (value) => {
  if (value === null || value === undefined || typeof value === 'string') {
    return (value as string | null) ?? '';
  }
  throw new Error('logout response shape is invalid');
};
