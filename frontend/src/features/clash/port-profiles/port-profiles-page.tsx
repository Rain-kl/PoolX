...
export function PortProfilesPage() {
  ...
  const [editingProfile, setEditingProfile] =
    useState<PortProfileWithNodes | null>(null);
  ...
  const [loadBalanceStrategy, setLoadBalanceStrategy] = useState<
  ...
  const fetchData = async (ignoreCheck?: unknown) => {
  ...
  const openEditDialog = (item: PortProfileWithNodes) => {
    ...
    setLoadBalanceStrategy(
      settings.load_balance_strategy || 'consistent-hashing',
    );
    ...
    setAdvancedOpen(
      Boolean(settings.auth_enabled || settings.udp_enabled === false),
    );
  ...
  const handlePreview = async (id: number) => {
    try {
      ...
      toast.error(
        `生成预览失败: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  };
      ...
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-sm text-muted-foreground'>
            定义代理监听入口、选择出口节点集合与策略（轮询/自动选优/故障转移）
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {profiles.map((item) => (
            <div
              key={item.profile.id}
              className='rounded-xl border bg-card p-5 shadow-sm space-y-4 hover:border-primary/50 transition-all'
            >
              <div className='flex items-center justify-between'>
                <span className='font-bold text-lg'>{item.profile.name}</span>
                <Badge
                  variant='outline'
                  className='uppercase font-mono text-xs bg-primary/5'
                >
              ...
              <div className='space-y-2 text-xs text-muted-foreground border-y py-3'>
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <span
                    className={
                      item.profile.include_in_runtime
              ...
              <div className='flex items-center justify-end gap-2 pt-1'>
                <Button
                  size='sm'
                  variant='ghost'
                  onClick={() => handlePreview(item.profile.id)}
                ...
                <Button
                  ...
                  onClick={() => handleDeleteClick(item.profile.id)}
                >
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div>
                ...
                <Select
                  value={strategyType}
                  onValueChange={(
                    val: 'select' | 'url-test' | 'fallback' | 'load-balance',
                  ) => setStrategyType(val)}
                >
            ...
            {strategyType === 'load-balance' && (
              <div className='space-y-1.5 bg-muted/40 p-3 rounded-lg border'>
                ...
                <Select
                  ...
                  onValueChange={(val: 'consistent-hashing' | 'round-robin') =>
                    setLoadBalanceStrategy(val)
                  }
                >
              ...
              <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-2'>
                ...
                <div className='flex items-center gap-1.5'>
                  <Button
                    ...
                    onClick={() =>
                      setSelectedNodeIds(nodesPool.map((n) => n.id))
                    }
                  >
                    全选
                  </Button>
                      ...
                      >
                        <label className='flex items-center gap-2 cursor-pointer select-none'>
                          <Checkbox
                            ...
                            onCheckedChange={(c) => {
                              if (c)
                                setSelectedNodeIds((prev) => [...prev, n.id]);
                              else
                                setSelectedNodeIds((prev) =>
                                  prev.filter((i) => i !== n.id),
                                );
                            }}
                          />
                          ...
                          <Badge
                            variant='secondary'
                            className='uppercase font-mono text-[10px] px-1 py-0'
                          >
                            {n.type}
                          </Badge>
                        </label>
            ...
            {previewResult && (
              <>
                <div className='flex items-center justify-between text-xs text-muted-foreground font-mono'>
                  <span>
                    Checksum: {previewResult.checksum.slice(0, 16)}...
                  </span>
