...
import { Upload, Link as LinkIcon, CheckCircle, Trash2, FileText, DownloadCloud, AlertCircle, FileCode, UploadCloud, X, RefreshCw } from 'lucide-react';
...
export function SourceConfigsPage() {
  ...
  const [previewData, setPreviewData] = useState<UploadSourceConfigResponse | null>(null);
  const [confirming, setConfirming] = useState(false);
  const [syncingId, setSyncingId] = useState<number | null>(null);

  // Reupload existing source
  const [reuploadTarget, setReuploadTarget] = useState<SourceConfig | null>(null);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      setUploading(true);
      const res = await uploadSourceConfig(filename || 'config.yaml', rawContent);
  ...
  const handleFetchSubscription = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(`拉取订阅失败: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
  ...
  const handleConfirmImport = async (id: number) => {
    try {
      ...
      toast.error(`确认导入失败: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setConfirming(false);
    }
  };
  ...
  const handleRefresh = async (id: number) => {
    try {
      ...
      toast.success(`刷新完成：入库 ${res.imported_nodes} 个节点（重复 ${res.duplicate_nodes}）`);
      await fetchSources();
    } catch (err) {
  ...
  const handleReupload = async (e: React.FormEvent) => {
    ...
    try {
      setReuploading(true);
      const res = await reuploadSourceConfig(reuploadTarget.id, reuploadFilename, reuploadContent);
      toast.success(`重新上传完成：入库 ${res.imported_nodes} 个节点（重复 ${res.duplicate_nodes}）`);
      setReuploadTarget(null);
      setReuploadContent('');
      await fetchSources();
    } catch (err) {
      toast.error(`重新上传失败: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground">配置源导入</h1>
          <p className="text-sm text-muted-foreground">支持拖拽/点击选择 YAML 文件、粘贴文本输入或通过 HTTP 订阅地址一键解析节点</p>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={() => setUploadDialogOpen(true)} className="gap-2">
            <Upload className="h-4 w-4" /> 上传 YAML 文件
          </Button>
          <Button onClick={() => setSubDialogOpen(true)} variant="outline" className="gap-2">
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">暂无配置源。请点击上方“上传 YAML 文件”或“拉取订阅 URL”。</TableCell>
              </TableRow>
            ) : (
              sources.map((item) => (
                <TableRow key={item.id}>
                  <TableCell>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {item.source_type === 'subscription_url' ? item.source_url : '本地上传'}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="text-xs space-y-0.5">
                      <div>有效节点: <span className="font-bold text-emerald-600">{item.valid_nodes}</span></div>
                      <div>重复节点: <span className="text-amber-600">{item.duplicate_nodes}</span></div>
                      <div>已入库: <span className="font-bold text-primary">{item.imported_nodes}</span></div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    {item.status === 'imported' ? (
                      <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20">已入库</Badge>
                    ) : (
                  ...
                  <TableCell className="text-right space-x-1">
                    {item.status !== 'imported' && (
                      <Button size="sm" onClick={() => handleConfirmImport(item.id)} disabled={confirming} className="gap-1">
                        <CheckCircle className="h-3.5 w-3.5" /> 确认导入
                      </Button>
                    )}
                    {item.status === 'imported' && item.source_type === 'subscription_url' && (
                      <Button
                        size="sm"
                        variant="outline"
                        className="gap-1"
                    ...
                    {item.status === 'imported' && item.source_type === 'upload' && (
                      <Button size="sm" variant="outline" className="gap-1" onClick={() => openReupload(item)}>
                        <Upload className="h-3.5 w-3.5" /> 重新上传
                      </Button>
                    )}
                    {item.status === 'imported' && (
                      <Button size="sm" variant="outline" onClick={() => handleConfirmImport(item.id)} disabled={confirming} className="gap-1">
                        <RefreshCw className="h-3.5 w-3.5" /> 重新同步
                      </Button>
                    )}
                    <Button size="sm" variant="ghost" className="text-destructive" onClick={() => handleDeleteClick(item.id)}>
          ...
          <Tabs value={uploadMode} onValueChange={(v: any) => setUploadMode(v)} className="w-full">
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              <TabsContent value="file" className="space-y-4 m-0">
                ...
                {!selectedFile ? (
                  ...
                  >
                    ...
                    <p className="font-semibold text-sm">点击选择文件，或将 .yaml / .yml 文件拖拽至此处</p>
                    <p className="text-xs text-muted-foreground mt-1">支持 Clash / Mihomo 配置文件</p>
                  </div>
                ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground font-mono">
                          {(selectedFile.size / 1024).toFixed(1)} KB | YAML 已读取 ({rawContent.length} 字符)
                        </div>
                      </div>
                    </div>
              ...
              <DialogFooter className="pt-2">
                ...
                <Button type="submit" disabled={uploading || !rawContent.trim()}>
      ...
      <Dialog open={subDialogOpen} onOpenChange={setSubDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleFetchSubscription} className="space-y-4 py-2">
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setSubDialogOpen(false)}>取消</Button>
              <Button type="submit" disabled={fetchingSub} className="gap-2">
                <DownloadCloud className="h-4 w-4" /> {fetchingSub ? '拉取中...' : '开始拉取'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      {/* Reupload Modal */}
      <Dialog open={!!reuploadTarget} onOpenChange={(open) => !open && setReuploadTarget(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
          ...
          <form onSubmit={handleReupload} className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              将覆盖「{reuploadTarget?.filename}」的内容，并以全删全插方式同步绑定节点。
            </p>
            <div>
              <label className="text-sm font-medium">配置名称 / 文件名</label>
              <Input
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setReuploadTarget(null)}>取消</Button>
              <Button type="submit" disabled={reuploading || !reuploadContent.trim()}>
          ...
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4 py-2 text-sm">
              <div className="grid grid-cols-4 gap-2 bg-muted p-3 rounded-lg text-center font-semibold">
                ...
                <div className="text-emerald-600">有效节点: {previewData.config.valid_nodes}</div>
                <div className="text-amber-600">重复节点: {previewData.config.duplicate_nodes}</div>
                <div className="text-rose-600">无效解析: {previewData.config.invalid_nodes}</div>
              </div>
              ...
              {previewData.parse_result.issues.length > 0 && (
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 space-y-1 text-xs text-amber-700">
                  ...
                  {previewData.parse_result.issues.map((issue, idx) => (
                    <div key={idx}>Line #{issue.index}: {issue.message}</div>
                  ))}
                </div>
              )}
                ...
                <div className="border rounded-md divide-y max-h-48 overflow-y-auto font-mono text-xs">
                  {previewData.parse_result.nodes.slice(0, 10).map((n, idx) => (
                    <div key={idx} className="p-2 flex justify-between">
                      <span className="font-bold truncate max-w-[200px]">{n.name}</span>
                      <span className="uppercase text-muted-foreground">{n.type}</span>
                      <span>{n.server}:{n.port}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setPreviewData(null)}>稍后确认</Button>
              <Button onClick={() => handleConfirmImport(previewData.config.id)} disabled={confirming} className="gap-2">
                <CheckCircle className="h-4 w-4" /> 确认入库 ({previewData.config.valid_nodes - previewData.config.duplicate_nodes} 新节点)
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
