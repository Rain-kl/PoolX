...
import { formatUSD } from '@/features/example/dashboard-format';
...
export function DashboardTopServices({
  ...
  return (
    <DashboardPanel
      id='dashboard-top-services-title'
          ...
          {loading ? (
            ...
            <TableRow className='hover:bg-transparent'>
              <TableCell colSpan={COLUMN_COUNT} className='p-0'>
                <EmptyState message={t('dashboard.noTopServices')} />
              </TableCell>
            </TableRow>
          ) : (
            services.map((item) => {
              ...
              return (
                <TableRow key={item.service} className='h-14'>
                  <TableCell>
                    <div className='min-w-0'>
                      ...
                      <p className='mt-1 truncate text-[10px] text-muted-foreground/80'>
                        {tokenDetails
                          .map(
                            ([label, value]) =>
                              `${label} ${formatNumber(Number(value), locale)}`,
                          )
                          .join(' · ')}
                      </p>
                    </div>
...
function formatCompactTokens(value: number, locale: string): string {
  ...
  const precision =
    Math.abs(compact) < 10 && !Number.isInteger(compact) ? 1 : 0;
