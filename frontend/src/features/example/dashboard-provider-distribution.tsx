...
type ProviderKey = 'primary' | 'edge' | 'backup';

const STRIPE_COUNT = 40;
const PROVIDERS: Array<{ key: ProviderKey; color: string; dot: string }> = [
  { key: 'primary', color: 'bg-quota-product-1', dot: 'bg-quota-product-1' },
  { key: 'edge', color: 'bg-quota-product-2', dot: 'bg-quota-product-2' },
  { key: 'backup', color: 'bg-quota-product-4', dot: 'bg-quota-product-4' },
];
...
export function DashboardProviderDistribution({
  ...
  const hoveredProvider =
    stripeHover === null ? null : (stripes[stripeHover.index] ?? null);
  const hoveredShare =
    hoveredProvider && totalRequests > 0
      ? (hoveredProvider.requests / totalRequests) * 100
      : 0;
        ...
        <div className='flex flex-1 flex-col'>
          ...
          <div className='mt-3 grid flex-1 grid-rows-3 divide-y'>
            {providers.map((provider) => {
              const share =
                totalRequests > 0
                  ? (provider.requests / totalRequests) * 100
                  : 0;
              const successRate =
                provider.requests > 0
...
function buildProviderStripes<T extends { requests: number }>(
  ...
  if (totalRequests <= 0)
    return Array.from({ length: STRIPE_COUNT }, () => null);
  ...
  return Array.from({ length: STRIPE_COUNT }, (_, index) => {
    ...
    const providerIndex = boundaries.findIndex(
      (boundary) => position <= boundary,
    );
    return (
      providers[providerIndex >= 0 ? providerIndex : providers.length - 1] ??
      null
    );
  });
}
...
function providerLabel(provider: ProviderKey, t: TFunction): string {
  if (provider === 'primary') return t('example.dashboard.providerPrimary');
  if (provider === 'edge') return t('example.dashboard.providerEdge');
  return t('example.dashboard.providerBackup');
}
